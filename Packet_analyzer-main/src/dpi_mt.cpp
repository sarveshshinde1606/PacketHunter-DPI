...
void DPIEngine::waitForCompletion() {
    // First wait until the reader has finished submitting packets.
    if (reader_thread_.joinable()) {
        reader_thread_.join();
    }

    // Do NOT use a fixed sleep here. With multiple FP/LB threads,
    // 500 ms is not a guarantee that every packet has finished.
    // Every processed packet must become either FORWARDED or DROPPED.
    const uint64_t total = stats_.total_packets.load();

    std::cout << "[DPIEngine] Waiting for all " << total
              << " packets to finish...\n";

    while (stats_.forwarded_packets.load() +
           stats_.dropped_packets.load() < total) {
        std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }

    std::cout << "[DPIEngine] All packets processed.\n";

    // The worker pipeline is now complete. The output thread can drain
    // any packets that are still sitting in its queue.
...
bool DPIEngine::processFile(const std::string& input_file,
    ...
    // Wait until every packet has been handled.
    waitForCompletion();

    // No arbitrary sleep is needed anymore.
    stop();
...
void DPIEngine::outputThreadFunc() {
    // FP threads finish packets in nondeterministic order. Collect all
    // forwarded packets first, then write them by their original packet_id.
    // This keeps the PCAP output deterministic without requiring dropped
    // packets to appear in the output.
    std::map<uint32_t, PacketJob> pending;
    ...
    while (running_ || !output_queue_.empty()) {
        ...
        pending.emplace(job_opt->packet_id, std::move(*job_opt));
    }

    // All workers are stopped and the output queue is drained. Now write
    // forwarded packets in the same order in which they appeared in input.
    for (auto& [packet_id, job] : pending) {
        (void)packet_id;
