...
std::cout << "[DPIEngine] Trying to write frontend stats...\n";
// Export statistics for React frontend
const char* stats_path = std::getenv("PACKETHUNTER_STATS_PATH");

if (!stats_path) {
    stats_path = "frontend/public/stats.json";
}

std::ofstream stats_file(stats_path);

if (stats_file.is_open()) {
    stats_file << generateStatsJson();
    stats_file.close();

const char* connections_path = std::getenv("PACKETHUNTER_CONNECTIONS_PATH");

if (!connections_path) {
    connections_path = "frontend/public/connections.json";
}

std::ofstream connections_file(connections_path);

    if (connections_file.is_open()) {
        connections_file << generateConnectionsJson();
        connections_file.close();

        std::cout << "[DPIEngine] Frontend statistics and connections updated\n";
    } else {
        std::cerr << "[DPIEngine] Warning: Could not write connections.json\n";
    }
}
else {

    std::cerr << "[DPIEngine] Warning: Could not write frontend stats\n";
}
...
std::string DPIEngine::generateStatsJson() const {
    std::ostringstream ss;

uint64_t active_connections = 0;
uint64_t lb_received = 0;
uint64_t lb_dispatched = 0;
uint64_t fp_processed = 0;
uint64_t fp_forwarded = 0;
uint64_t fp_dropped = 0;

if (lb_manager_) {
    auto lb_stats = lb_manager_->getAggregatedStats();
    lb_received = lb_stats.total_received;
    lb_dispatched = lb_stats.total_dispatched;
}

if (fp_manager_) {
    auto fp_stats = fp_manager_->getAggregatedStats();

    active_connections = fp_stats.total_connections;
    fp_processed = fp_stats.total_processed;
    fp_forwarded = fp_stats.total_forwarded;
    fp_dropped = fp_stats.total_dropped;
}

    ss << "{\n";
    ss << "  \"totalPackets\": " << stats_.total_packets.load() << ",\n";
    ss << "  \"totalBytes\": " << stats_.total_bytes.load() << ",\n";
    ss << "  \"forwardedPackets\": " << stats_.forwarded_packets.load() << ",\n";
    ss << "  \"droppedPackets\": " << stats_.dropped_packets.load() << ",\n";
    ss << "  \"tcpPackets\": " << stats_.tcp_packets.load() << ",\n";
    ss << "  \"udpPackets\": " << stats_.udp_packets.load() << ",\n";
    ss << "  \"otherPackets\": " << stats_.other_packets.load() << ",\n";
    ss << "  \"activeConnections\": " << active_connections << ",\n";
    ss << "  \"lbReceived\": " << lb_received << ",\n";
ss << "  \"lbDispatched\": " << lb_dispatched << ",\n";
ss << "  \"fpProcessed\": " << fp_processed << ",\n";
ss << "  \"fpForwarded\": " << fp_forwarded << ",\n";
ss << "  \"fpDropped\": " << fp_dropped << ",\n";
...
if (rule_manager_) {
    ...
    ss << "    \"blockedIPs\": " << rule_stats.blocked_ips << ",\n";
    ss << "    \"blockedApps\": " << rule_stats.blocked_apps << ",\n";
    ss << "    \"blockedDomains\": " << rule_stats.blocked_domains << ",\n";
    ss << "    \"blockedPorts\": " << rule_stats.blocked_ports << "\n";
} else {
    ss << "    \"blockedIPs\": 0,\n";
    ss << "    \"blockedApps\": 0,\n";
    ss << "    \"blockedDomains\": 0,\n";
...
std::string DPIEngine::generateConnectionsJson() const {
    std::ostringstream ss;
    ...
    // Convert uint32_t IP to dotted decimal
        ...
        for (int i = 0; i < fp_manager_->getNumFPs(); ++i) {
            ...
            tracker.forEach([&](const Connection& conn) {
                ...
                switch (conn.state) {
                    ...
                    case ConnectionState::CLASSIFIED:
                        status = "CLASSIFIED";
                        break;
                ...
                switch (conn.action) {
                    ...
                    case PacketAction::INSPECT:
                        action = "INSPECT";
                        break;

                    case PacketAction::LOG_ONLY:
                        action = "LOG";
                        break;
                ...
                std::string source =
                    formatIP(conn.tuple.src_ip) +
                    ":" +
                    std::to_string(conn.tuple.src_port);

                std::string destination =
                    formatIP(conn.tuple.dst_ip) +
                    ":" +
                    std::to_string(conn.tuple.dst_port);
...
if (protocol == "TCP") {
    ...
    // Escape characters that are not safe inside JSON strings
    std::string escaped;
    for (unsigned char c : safeSni) {
        switch (c) {
            case '\"':
                escaped += "\\\"";
                break;
            ...
            default:
                if (c >= 32 && c <= 126) {
                    ...
                    // Ignore binary/non-printable bytes
                }
                break;
        }
    }
